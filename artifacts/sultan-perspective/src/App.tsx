...
const formatMoney = (amount: number, currency: CurrencyCode, locale: Locale) => {
  const formatted = new Intl.NumberFormat(locale === 'ar' ? 'ar-LB' : 'en-US', {
    maximumFractionDigits: 0,
  }).format(Math.round(amount * currencies[currency].rate));
...
function App() {
  ...
  const [packageAddOns, setPackageAddOns] = useState<Record<string, string[]>>({
    standard: [],
    pro: [],
    ultimate: [],
  });
  ...
  const estimateForPackage = (packageId: string) => {
    const packageOffer = packages.find((item) => item.id === packageId) ?? packages[1];
    return (packageOffer.price + addOnTotal(packageAddOns[packageId] ?? [])) * currencies[currency].rate;
  };
  const total = useMemo(
    () => (chosenPackage.price + addOnTotal(selectedOptions)) * currencies[currency].rate,
    [chosenPackage.price, currency, selectedOptions],
  );
      ...
      <header className="absolute inset-x-0 top-0 z-40 border-b border-white/10">
        <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          ...
          <div className="hidden items-center gap-4 sm:flex">
            ...
            <a href={`tel:${brand.whatsapp}`} className="text-[11px] text-[#f5eee3]/55 hover:text-[#d29a38]" data-testid="link-header-phone">
              {brand.phone}
            </a>
        ...
        {menuOpen && (
          <div className="border-t border-white/10 bg-[#1d2027] px-5 py-6 lg:hidden">
            <nav className="flex flex-col gap-5" aria-label={getText(copy.nav.services, locale)}>
              ...
              <a href={`tel:${brand.whatsapp}`} className="border-t border-white/10 pt-5 text-xs text-[#d29a38]" data-testid="link-mobile-phone">
                {brand.phone}
              </a>
            </nav>
          </div>
      ...
      <main>
          ...
          <div className="relative mx-auto flex min-h-[760px] max-w-[1440px] flex-col justify-end px-5 pb-12 pt-36 sm:min-h-[820px] sm:px-8 sm:pb-16 lg:px-12 lg:pb-20">
            <div className="max-w-[960px]">
              ...
              <h1 className="display-font reveal reveal-delay-1 max-w-[920px] text-[clamp(3.2rem,8.4vw,8.3rem)] font-medium leading-[.9] tracking-[-.075em]">
                {isArabic ? (
                  <>
                    ارفع قيمة
                    <span className="block text-[#d29a38]">عقاراتك.</span>
                  </>
                ) : (
                  <>
                    Elevate Your
              ...
              <div className="reveal reveal-delay-2 mt-8 grid max-w-[850px] gap-8 sm:grid-cols-[1fr_auto] sm:items-end">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    aria-label={getText(copy.hero.playReel, locale)}
            ...
            <div className="reveal reveal-delay-3 mt-14 grid grid-cols-2 gap-px border border-white/10 bg-white/10 sm:grid-cols-4">
              {proofPoints.map((point) => (
                <div className="bg-[#1d2027]/80 px-4 py-4 sm:px-5" key={point.label.en} data-testid={`stat-proof-${point.label.en.replaceAll(' ', '-')}`}>
          ...
          <div className="grid gap-14 lg:grid-cols-[.75fr_1.25fr] lg:gap-24">
            <div>
              <div className="mono-label flex items-center gap-3 text-[10px] text-[#ad6f18]">
                <span className="size-2 bg-[#d29a38]" /> {getText(copy.services.eyebrow, locale)}
              </div>
              <h2 className="display-font mt-6 max-w-[480px] text-5xl leading-[.98] tracking-[-.06em] sm:text-7xl">
                {getText(copy.services.titleLead, locale)} <span className="text-[#b9791d]">{getText(copy.services.titleAccent, locale)}</span>
              </h2>
              ...
              <a href="#approach" className="group mt-8 inline-flex items-center gap-2 border-b border-[#1d2027] pb-2 text-[11px] font-bold uppercase tracking-[.15em]" data-testid="link-services-approach">
                {getText(copy.services.link, locale)} <ArrowUpRight className="size-4 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
              </a>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              {services.map((service, index) => (
                <button
                  ...
                  className={`service-card group min-h-[230px] border p-6 text-start sm:p-7 ${
                ...
                >
                  ...
                  <div className="mt-12 flex items-end justify-between gap-4">
                    <div>
                      <h3 className="display-font text-2xl leading-none">{getText(service.title, locale)}</h3>
                      <p className={`mt-3 text-xs leading-5 ${activeService === index ? 'text-[#f5eee3]/60' : 'text-[#68676a]'}`}>{getText(service.text, locale)}</p>
                    </div>
        ...
        <section id="approach" className="relative overflow-hidden bg-[#20242c] text-[#f5eee3]">
          ...
          <div className="relative mx-auto grid max-w-[1440px] gap-14 px-5 py-24 sm:px-8 lg:grid-cols-[1fr_1fr] lg:gap-24 lg:px-12 lg:py-32">
            <div>
              <div className="mono-label text-[10px] text-[#d29a38]">{getText(copy.approach.eyebrow, locale)}</div>
              <h2 className="display-font mt-6 max-w-[590px] text-5xl leading-[.95] tracking-[-.06em] sm:text-7xl">
                {getText(copy.approach.titleLead, locale)} <span className="text-[#d29a38]">{getText(copy.approach.titleAccent, locale)}</span>
              </h2>
              <div className="gold-rule mt-9 max-w-[320px]" />
              <p className="mt-8 max-w-[500px] text-[15px] leading-7 text-[#f5eee3]/60">{getText(copy.approach.description, locale)}</p>
            </div>
            <div className="grid gap-0 border-s border-white/15 ps-6 sm:ps-10">
              {copy.approach.steps.map((step) => (
                <div key={step.number} className="group border-b border-white/15 py-7 first:pt-0 last:border-0">
                  <div className="flex items-start gap-6">
                    <span className="mono-label pt-1 text-[10px] text-[#d29a38]">{step.number}</span>
                    <div>
                      <h3 className="display-font text-2xl">{getText(step.title, locale)}</h3>
                      <p className="mt-2 max-w-[360px] text-sm leading-6 text-[#f5eee3]/50">{getText(step.text, locale)}</p>
                    </div>
                    <ArrowUpRight className="ms-auto size-4 text-[#d29a38] opacity-0 transition-opacity group-hover:opacity-100" />
                  </div>
                </div>
              ))}
            </div>
        ...
        <section id="pricing" className="mx-auto max-w-[1440px] px-5 py-24 sm:px-8 lg:px-12 lg:py-36">
          <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
            <div>
              <div className="mono-label flex items-center gap-3 text-[10px] text-[#ad6f18]"><span className="size-2 bg-[#d29a38]" /> {getText(copy.pricing.eyebrow, locale)}</div>
              <h2 className="display-font mt-5 text-5xl tracking-[-.06em] sm:text-7xl">
                {getText(copy.pricing.titleLead, locale)} <span className="text-[#b9791d]">{getText(copy.pricing.titleAccent, locale)}</span>
              </h2>
            </div>
            <div className="flex flex-col items-start gap-4 sm:items-end">
              <p className="max-w-[260px] text-sm leading-6 text-[#68676a] sm:text-end">{getText(copy.pricing.description, locale)}</p>
            ...
            {packages.map((item, index) => {
              ...
              return (
                <article key={item.id} className={`package-card relative flex flex-col border p-7 sm:p-8 ${item.tone === 'gold' ? 'border-[#b9791d] bg-[#d29a38]' : item.tone === 'dark' ? 'border-[#1d2027] bg-[#1d2027] text-[#f5eee3]' : 'border-[#d5cfc4] bg-[#ebe5da]'}`} data-testid={`card-package-${item.id}`}>
                  {item.id === 'pro' && <div className="absolute -top-3 start-7 bg-[#1d2027] px-3 py-1 text-[9px] font-bold uppercase tracking-[.16em] text-[#d29a38]">{getText(copy.pricing.mostRequested, locale)}</div>}
                  <div className="flex items-start justify-between">
                  ...
                  <div className={`mt-8 flex items-end gap-2 border-b pb-6 ${item.tone === 'dark' ? 'border-white/15' : 'border-[#1d2027]/20'}`}>
                    ...
                    <span className={`pb-1 text-[10px] ${item.tone === 'dark' ? 'text-[#f5eee3]/50' : 'text-[#1d2027]/50'}`}>{getText(item.unit, locale)}</span>
                  </div>
                  ...
                  <div className={`mt-7 border-t pt-5 ${item.tone === 'dark' ? 'border-white/15' : 'border-[#1d2027]/20'}`}>
                        ...
                        return (
                          <label key={addon.id} className={`flex cursor-pointer items-center gap-3 text-xs ${item.tone === 'dark' ? 'text-[#f5eee3]/75' : 'text-[#1d2027]/75'}`}>
                            <input
                              ...
                              className="sr-only"
                              checked={checked}
                              onChange={() => togglePackageAddOn(item.id, addon.id)}
                              data-testid={`checkbox-${item.id}-${addon.id}`}
                            />
                            <span className={`grid size-4 shrink-0 place-items-center border ${checked ? 'border-[#1d2027] bg-[#1d2027] text-[#d29a38]' : item.tone === 'dark' ? 'border-white/35' : 'border-[#9e9588]'}`}>
                    ...
                    <div className={`mt-5 flex items-baseline justify-between gap-3 border-t pt-4 ${item.tone === 'dark' ? 'border-white/15' : 'border-[#1d2027]/15'}`}>
                      ...
                      <span className="display-font text-2xl">{formatMoney(estimateForPackage(item.id), currency, locale)}</span>
                    </div>
                  </div>
        ...
        <section className="border-y border-[#d5cfc4] bg-[#e8e1d6]">
          <div className="mx-auto grid max-w-[1440px] gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[.75fr_1.25fr] lg:px-12">
            <div>
              <div className="mono-label text-[10px] text-[#ad6f18]">{getText(copy.estimator.eyebrow, locale)}</div>
              <h2 className="display-font mt-5 text-4xl tracking-[-.05em] sm:text-5xl">
                {getText(copy.estimator.titleLead, locale)} <span className="text-[#b9791d]">{getText(copy.estimator.titleAccent, locale)}</span>
              </h2>
              <p className="mt-4 max-w-[390px] text-sm leading-6 text-[#68676a]">{interpolate(getText(copy.estimator.description, locale), { package: activePackageName })}</p>
            </div>
            <div className="border border-[#cfc7ba] bg-[#f1ece3]">
              <div className="flex items-center justify-between border-b border-[#d5cfc4] px-5 py-4">
                <span className="mono-label text-[9px] text-[#68676a]">{getText(copy.estimator.estimateLabel, locale)} · {activePackageName}</span>
                <span className="display-font text-2xl">{formatMoney(total, currency, locale)}</span>
              </div>
              <div className="divide-y divide-[#d5cfc4]">
                {addOns.map((addon) => {
                  const checked = selectedOptions.includes(addon.id);
                  return (
                    <label key={addon.id} className="flex w-full cursor-pointer items-center gap-4 px-5 py-5 text-start transition-colors hover:bg-[#e8e1d6]">
                      <input
                        type="checkbox"
                        className="sr-only"
                        checked={checked}
                        onChange={() => togglePackageAddOn(selectedPackage, addon.id)}
                        data-testid={`checkbox-estimator-${addon.id}`}
                      />
                      ...
                      <span className="flex-1"><span className="block text-sm font-semibold">{getText(addon.name, locale)}</span><span className="mt-1 block text-xs text-[#7e7b78]">{getText(addon.note, locale)}</span></span>
                      <span className="text-sm font-semibold">+{formatMoney(addon.price, currency, locale)}</span>
                    </label>
                  );
                })}
              </div>
              <div className="flex flex-col gap-4 border-t border-[#d5cfc4] px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
                <span className="text-xs leading-5 text-[#68676a]">{getText(copy.estimator.confirmation, locale)}</span>
                <button type="button" onClick={() => scrollToBooking()} className="group inline-flex items-center gap-2 text-start text-[10px] font-bold uppercase tracking-[.16em]" data-testid="button-estimator-book">
                  {getText(copy.estimator.continue, locale)} <ArrowUpRight className="size-4 text-[#ad6f18] transition-transform group-hover:translate-x-1 group-hover:-translate-y-1" />
                </button>
              </div>
            </div>
          </div>
        ...
        <section className="mx-auto max-w-[1440px] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_.9fr]">
            <div className="relative min-h-[420px] overflow-hidden bg-[#1d2027] p-7 text-[#f5eee3] sm:p-10">
              <img src={locale === 'ar' ? '/assets/price-list-ar.png' : '/assets/price-list-en.png'} alt={getText(copy.reference.alt, locale)} className="absolute inset-0 h-full w-full object-cover opacity-25 mix-blend-screen" data-testid="img-price-reference" />
              <div className="absolute inset-0 bg-gradient-to-tr from-[#1d2027] via-[#1d2027]/75 to-transparent" />
              <div className="relative flex h-full flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="mono-label text-[9px] text-[#d29a38]">{getText(copy.reference.eyebrow, locale)}</span>
                  <span className="border border-[#f5eee3]/25 px-2 py-1 text-[9px] text-[#f5eee3]/50">2026 / {getText(copy.reference.language, locale)}</span>
                </div>
                <div>
                  <h2 className="display-font max-w-[490px] text-4xl leading-[.98] tracking-[-.05em] sm:text-6xl">
                    {getText(copy.reference.titleLead, locale)} <span className="text-[#d29a38]">{getText(copy.reference.titleAccent, locale)}</span>
                  </h2>
                  <p className="mt-5 max-w-[420px] text-sm leading-6 text-[#f5eee3]/60">{getText(copy.reference.description, locale)}</p>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {copy.reference.proof.map((item) => {
                const Icon = item.icon === 'plane' ? Plane : item.icon === 'camera' ? Camera : item.icon === 'orbit' ? Orbit : Grid3X3;
                return (
                  <div key={item.label.en} className="group flex min-h-[200px] flex-col justify-between border border-[#d5cfc4] bg-[#ebe5da] p-5 transition-colors hover:border-[#b9791d] sm:p-7" data-testid={`card-proof-${item.label.en.replace(' ', '-')}`}>
                    ...
                    <div><div className="mono-label text-[9px] text-[#8e887f]">{getText(item.label, locale)}</div><div className="display-font mt-2 text-xl leading-tight">{getText(item.title, locale)}</div></div>
                  </div>
                );
              })}
            </div>
        ...
        <section id="booking" className="bg-[#d29a38]">
          <div className="mx-auto grid max-w-[1440px] gap-14 px-5 py-24 sm:px-8 lg:grid-cols-[.8fr_1.2fr] lg:gap-24 lg:px-12 lg:py-32">
            <div>
              <div className="mono-label text-[10px] text-[#1d2027]/60">{getText(copy.booking.eyebrow, locale)}</div>
              <h2 className="display-font mt-6 max-w-[530px] text-5xl leading-[.92] tracking-[-.065em] sm:text-7xl">
                {getText(copy.booking.titleLead, locale)} <span className="text-[#f1ece3]">{getText(copy.booking.titleAccent, locale)}</span>
              </h2>
              <p className="mt-7 max-w-[420px] text-sm leading-6 text-[#1d2027]/70">{getText(copy.booking.description, locale)}</p>
              <div className="mt-12 space-y-4 border-t border-[#1d2027]/20 pt-6">
                ...
                <div className="flex items-center gap-3 text-sm font-semibold"><MapPin className="size-4" /> {getText(brand.location, locale)}</div>
              </div>
            </div>
            <div className="bg-[#f1ece3] p-6 sm:p-9">
              {submitted ? (
                <div className="flex min-h-[480px] flex-col items-center justify-center text-center">
                  ...
                  <div className="mono-label mt-7 text-[10px] text-[#ad6f18]">{getText(copy.booking.received, locale)}</div>
                  <h3 className="display-font mt-4 text-4xl tracking-[-.05em]">{getText(copy.booking.receivedTitle, locale)}</h3>
                  <p className="mt-4 max-w-[360px] text-sm leading-6 text-[#68676a]">
                    {interpolate(getText(copy.booking.receivedText, locale), { name: form.name.split(' ')[0] || (isArabic ? 'صديقي' : 'there') })}
                  </p>
                  <button type="button" onClick={() => { setSubmitted(false); setForm(initialForm); setPackageAddOns({ standard: [], pro: [], ultimate: [] }); }} className="mt-8 border-b border-[#1d2027] pb-2 text-[10px] font-bold uppercase tracking-[.15em]" data-testid="button-submit-another">
                    {getText(copy.booking.another, locale)}
                  </button>
                </div>
              ) : (
                <form onSubmit={submitBooking} noValidate>
                  <div className="mb-8 flex items-center justify-between border-b border-[#d5cfc4] pb-5">
                    <span className="mono-label text-[10px] text-[#68676a]">{getText(copy.booking.projectDetails, locale)}</span>
                    <span className="text-xs text-[#8a8781]">{getText(copy.booking.step, locale)}</span>
                  </div>
                  <div className="grid gap-5 sm:grid-cols-2">
                    <Field label={getText(copy.booking.fields.name, locale)} error={errors.name}><input value={form.name} onChange={(event) => updateField('name', event.target.value)} placeholder={getText(copy.booking.placeholders.name, locale)} data-testid="input-name" /></Field>
                    <Field label={getText(copy.booking.fields.email, locale)} error={errors.email}><input type="email" value={form.email} onChange={(event) => updateField('email', event.target.value)} placeholder={getText(copy.booking.placeholders.email, locale)} data-testid="input-email" /></Field>
                    <Field label={getText(copy.booking.fields.phone, locale)} error={errors.phone}><input type="tel" value={form.phone} onChange={(event) => updateField('phone', event.target.value)} placeholder={getText(copy.booking.placeholders.phone, locale)} data-testid="input-phone" /></Field>
                    <Field label={getText(copy.booking.fields.date, locale)} error={errors.date}><input type="date" value={form.date} onChange={(event) => updateField('date', event.target.value)} data-testid="input-date" /></Field>
                    <div className="sm:col-span-2"><Field label={getText(copy.booking.fields.address, locale)} error={errors.address}><input value={form.address} onChange={(event) => updateField('address', event.target.value)} placeholder={getText(copy.booking.placeholders.address, locale)} data-testid="input-address" /></Field></div>
                    <Field label={getText(copy.booking.fields.squareFootage, locale)} error={errors.squareFootage}><input type="number" min="1" value={form.squareFootage} onChange={(event) => updateField('squareFootage', event.target.value)} placeholder={getText(copy.booking.placeholders.squareFootage, locale)} data-testid="input-square-footage" /></Field>
                    <Field label={getText(copy.booking.fields.package, locale)} error={errors.packageId}>
                      <div className="relative">
                        <select value={form.packageId} onChange={(event) => { updateField('packageId', event.target.value); setSelectedPackage(event.target.value); }} data-testid="select-package">
                          {packages.map((item) => <option value={item.id} key={item.id}>{getText(item.name, locale)} — {formatMoney(item.price, currency, locale)}</option>)}
                        </select>
                        <ChevronDown className={`pointer-events-none absolute top-1/2 size-4 -translate-y-1/2 text-[#8a8781] ${isArabic ? 'left-3' : 'right-3'}`} />
                      </div>
                    </Field>
                  </div>
                  <div className="mt-8 flex flex-col justify-between gap-5 border-t border-[#d5cfc4] pt-6 sm:flex-row sm:items-center">
                    <p className="max-w-[260px] text-xs leading-5 text-[#8a8781]">{getText(copy.booking.disclaimer, locale)}</p>
                    <button type="submit" className="group inline-flex items-center justify-center gap-3 bg-[#1d2027] px-6 py-4 text-[10px] font-bold uppercase tracking-[.16em] text-[#f5eee3] transition-colors hover:bg-[#ad6f18]" data-testid="button-submit-booking">
                      {getText(copy.booking.submit, locale)} <Send className="size-4 transition-transform group-hover:translate-x-1" />
                    </button>
                  </div>
                </form>
              )}
      ...
      <footer className="bg-[#1d2027] text-[#f5eee3]">
        <div className="mx-auto max-w-[1440px] px-5 py-12 sm:px-8 lg:px-12">
          <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-[1.3fr_1fr_1fr_1fr]">
            <div><Logo /><p className="mt-6 max-w-[250px] text-sm leading-6 text-[#f5eee3]/45">{getText(copy.footer.description, locale)}</p></div>
            <div><div className="mono-label text-[9px] text-[#d29a38]">{getText(copy.footer.explore, locale)}</div><div className="mt-5 flex flex-col gap-3 text-sm text-[#f5eee3]/65"><a href="#services" className="hover:text-[#d29a38]" data-testid="link-footer-services">{getText(copy.nav.services, locale)}</a><a href="#pricing" className="hover:text-[#d29a38]" data-testid="link-footer-pricing">{getText(copy.nav.pricing, locale)}</a><a href="#approach" className="hover:text-[#d29a38]" data-testid="link-footer-approach">{getText(copy.nav.approach, locale)}</a></div></div>
            <div><div className="mono-label text-[9px] text-[#d29a38]">{getText(copy.footer.contact, locale)}</div><div className="mt-5 flex flex-col gap-3 text-sm text-[#f5eee3]/65"><a href={`tel:${brand.whatsapp}`} className="hover:text-[#d29a38]" data-testid="link-footer-phone">{brand.phone}</a><a href="#booking" className="hover:text-[#d29a38]" data-testid="link-footer-book">{getText(copy.nav.booking, locale)}</a><span>{getText(brand.location, locale)}</span></div></div>
            <div><div className="mono-label text-[9px] text-[#d29a38]">{getText(copy.footer.follow, locale)}</div><a href={`https://instagram.com/${brand.instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm text-[#f5eee3]/65 hover:text-[#d29a38]" data-testid="link-footer-instagram"><Instagram className="size-4" /> {brand.instagram}</a></div>
          </div>
          <div className="mt-14 flex flex-col justify-between gap-3 border-t border-white/10 pt-5 text-[10px] text-[#f5eee3]/30 sm:flex-row"><span>{getText(copy.footer.reserved, locale)}</span><span className="mono-label text-[8px]">{getText(copy.footer.tagline, locale)}</span></div>
        </div>
      </footer>

      {storyOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-[#1d2027]/90 p-5" role="dialog" aria-modal="true" aria-label={getText(copy.hero.playReel, locale)}>
          <button type="button" className="absolute inset-0 cursor-default" onClick={() => setStoryOpen(false)} aria-label={getText(copy.reel.close, locale)} data-testid="button-close-reel-backdrop" />
          <div className="relative z-10 w-full max-w-3xl border border-[#d29a38]/50 bg-[#20242c] p-6 text-[#f5eee3] sm:p-10">
            <button type="button" onClick={() => setStoryOpen(false)} className="absolute end-4 top-4 text-[#f5eee3]/60 hover:text-[#d29a38]" aria-label={getText(copy.reel.close, locale)} data-testid="button-close-reel"><X className="size-5" /></button>
            <div className="hero-grid relative flex min-h-[360px] items-center justify-center overflow-hidden border border-white/10">
              ...
              <div className="relative text-center"><Plane className="mx-auto size-9 text-[#d29a38]" strokeWidth={1.2} /><div className="display-font mt-5 text-4xl tracking-[-.05em]">{getText(copy.reel.titleLead, locale)} <span className="text-[#d29a38]">{getText(copy.reel.titleAccent, locale)}</span></div><div className="mono-label mt-4 text-[9px] text-[#f5eee3]/45">{getText(copy.reel.label, locale)}</div></div>
            </div>
          </div>
        </div>
      )}
